...
`;function Pt(e){return e instanceof Element}customElements.define("project-detail",et((function({projectName:e}){const t=lt.find((t=>st(t.title)===e));return t?de`
  ...
  `:de`<div>No project found</div>`}))),customElements.define("app-component",et((function(){const[e,t]=Ye(""),[i,s]=Ye(""),[o,n]=Ye(void 0);Ke((()=>{ot({setNavFocus:t,setProject:r});const e=["#summary","#visuals","#experience"].map((e=>this.shadowRoot?.querySelector(e))).filter(Pt);for(const t of e)it({element:t,onIntersection:a,unobserve:!1,options:{threshold:.05}})}),[]),Ke((()=>{!e||i||o||function(e,t,i="instant"){if(!t)return;const s=e.querySelector(t),o=e.querySelector(nt.HEADER),n=o?o.offsetHeight:0;if(s){const e=s.offsetTop-n;window.scroll({top:e,behavior:i})}}(this.shadowRoot,e)}),[e,i,o]),Ke((()=>{i||null==o||scrollTo({top:o})}),[o,i]);const r=e=>{if(e){n(window.scrollY);const t=lt.find((t=>st(t.title)===e))?.title;document.title=`Jory's ${t} Project`}else document.title="Jory Phillips Portfolio and Resume";s(e)},a=e=>{history.replaceState(null,"",`#${e.id}`),t(`#${e.id}`),n(window.scrollY)};return de`
    ${tt}
